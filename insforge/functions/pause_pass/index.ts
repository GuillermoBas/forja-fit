...
export default async function(request: Request) {
  try {
    ...
    if (!body?.passId || !body?.startsOn || !body?.endsOn) {
      return json({ code: "INVALID_INPUT", message: "Faltan datos de pausa" }, 400)
    }
    ...
    const rpcResult = await client.database.rpc("app_pause_pass", {
    ...
    return json({ ok: true, passPauseId: rpcResult.data })
  } catch (error) {
    return json(
