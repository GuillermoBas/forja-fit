...
export default async function(request: Request) {
  try {
    ...
    if (!body?.passId || !body?.clientId) {
      return json({ code: "INVALID_INPUT", message: "Faltan passId o clientId" }, 400)
    }
    ...
    const rpcResult = await client.database.rpc("app_consume_session", {
      p_actor_profile_id: actor.profile.id,
      p_pass_id: body.passId,
      p_client_id: body.clientId,
      p_consumed_at: body.consumedAt ?? new Date().toISOString(),
      p_notes: body.notes ?? ""
    })
    ...
    return json({ ok: true, sessionConsumptionId: rpcResult.data })
  } catch (error) {
    return json(
