...
async function getActor(client: any, gymId: string) {
  ...
  if (profileResult.data.role !== "admin") {
    return { error: json({ code: "FORBIDDEN", message: "Solo admin puede anadir stock" }, 403) }
  }
...
export default async function(request: Request) {
  try {
    ...
    if (!body?.productId || !body?.quantity || Number(body.quantity) <= 0) {
      return json({ code: "INVALID_INPUT", message: "Producto y cantidad son obligatorios" }, 400)
    }
    ...
    const rpcResult = await client.database.rpc("app_add_stock", {
