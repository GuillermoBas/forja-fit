...
async function getActor(client: any) {
  ...
  if (profileResult.data.role !== "admin") {
    return { error: json({ code: "FORBIDDEN", message: "Solo admin puede reducir stock" }, 403) }
  }
...
export default async function(request: Request) {
  try {
    ...
    if (!body?.productId || !body?.quantity || Number(body.quantity) <= 0 || !String(body.reason ?? "").trim()) {
      return json({ code: "INVALID_INPUT", message: "Producto, cantidad y motivo son obligatorios" }, 400)
    }
    ...
    const rpcResult = await client.database.rpc("app_reduce_stock", {
    ...
    return json({
      ok: true,
      stock: rpcResult.data?.stock_on_hand ?? null
    })
  } catch (error) {
    return json(
