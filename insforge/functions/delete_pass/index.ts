...
async function getActor(client: any, gymId: string) {
  ...
  if (profileResult.data.role !== "admin") {
    return { error: json({ code: "FORBIDDEN", message: "Solo admin puede borrar bonos" }, 403) }
  }
...
export default async function(request: Request) {
  try {
    ...
    if (!body?.passId) {
      return json({ code: "INVALID_INPUT", message: "El bono es obligatorio" }, 400)
    }
    ...
    const rpcResult = await client.database.rpc("app_delete_pass", {
      p_actor_profile_id: actor.profile.id,
      p_pass_id: body.passId
    })

    if (rpcResult.error) {
      return json({ code: "DB_ERROR", message: rpcResult.error.message || "No se pudo borrar el bono" }, 400)
    }
