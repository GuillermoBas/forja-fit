...
async function getActor(client: any) {
  ...
  if (profileResult.data.role !== "admin") {
    return { error: json({ code: "FORBIDDEN", message: "Solo admin puede borrar tipos de bono" }, 403) }
  }
...
export default async function(request: Request) {
  try {
    ...
    if (!body?.passTypeId || typeof body.passTypeId !== "string") {
      return json({ code: "INVALID_INPUT", message: "Falta el tipo de bono a borrar" }, 400)
    }
    ...
    const rpcResult = await client.database.rpc("app_delete_pass_type", {
    ...
    if (rpcResult.error) {
      return json(
        { code: "DB_ERROR", message: rpcResult.error.message || "No se pudo borrar el tipo de bono" },
        400
      )
    }
