...
export default async function(request: Request) {
  try {
    ...
    if (!body?.profileId || !body?.calendarColor) {
      return json({ code: "INVALID_INPUT", message: "Faltan datos del color de agenda" }, 400)
    }
    ...
    const rpcResult = await client.database.rpc("app_update_profile_calendar_color", {
      p_actor_profile_id: actor.profile.id,
      p_target_profile_id: body.profileId,
      p_calendar_color: body.calendarColor
    })
