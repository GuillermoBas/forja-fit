...
async function getActor(token: string, gymId: string) {
  ...
  if (profile.role !== "admin" && profile.role !== "trainer") {
    return { error: json({ code: "FORBIDDEN", message: "Solo staff puede borrar maximos de fuerza" }, 403) }
  }
...
export default async function(request: Request) {
  try {
    ...
    const rpcResult = await insforgeFetch(
      "/api/database/rpc/app_delete_client_max_weight_entry",
      token,
      {
        method: "POST",
        body: JSON.stringify({
    ...
    if (rpcResult.error) {
      return json(
        { code: "DB_ERROR", message: rpcResult.error || "No se pudo borrar el maximo de fuerza" },
        400
      )
    }
