ALTER TABLE client_nutrition_profiles
  ...
  ADD COLUMN IF NOT EXISTS intermittent_fasting BOOLEAN,
  ...
  ADD COLUMN IF NOT EXISTS usual_schedule TEXT,
  ADD COLUMN IF NOT EXISTS rolling_summary TEXT,
  ADD COLUMN IF NOT EXISTS rolling_summary_message_count INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS rolling_summary_refreshed_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS rolling_summary_model_id TEXT;

ALTER TABLE client_nutrition_profiles
  DROP CONSTRAINT IF EXISTS client_nutrition_profiles_meals_per_day_check;

ALTER TABLE client_nutrition_profiles
  ADD CONSTRAINT client_nutrition_profiles_meals_per_day_check
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'thread_id', v_thread_id,
    'quota', app_get_client_nutrition_quota_status(p_auth_user_id),
...
GRANT EXECUTE ON FUNCTION app_get_client_nutrition_quota_status(UUID) TO authenticated;
